...
const Navbar = () => {
  ...
  return (
    <div className="nav fixed z-999 w-full px-6 md:px-16 py-5 flex items-center justify-between">
      <div className="logo">
        <svg
          width="72"
          height="30"
        ...
        {["Services", "Our work", "About us", "Insights", "Contact us"].map(
          function (elem, idx) {
            return (
              <a
                key={idx}
                className={`text-[18px] text-black font-small ${idx === 4 && "md:ml-20 lg:ml-40"}`}
              >
                {elem}
              </a>
            );
      ...
      >
        <span
          className={`block h-[2px] w-6 bg-black transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-[7px]" : ""}`}
        />
        <span
      ...
      {menuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-md flex flex-col items-start px-6 py-6 gap-5 shadow-lg">
          {["Services", "Our work", "About us", "Insights", "Contact us"].map(
            (elem, idx) => (
              <a
                key={idx}
                className="text-[18px] text-black w-full border-b border-gray-200 pb-3 last:border-0"
